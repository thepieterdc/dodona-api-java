...
public class SubmissionManagerImpl implements SubmissionManager {
	...
	public long create(@Nullable final Course course,
		...
		final Long courseId = Optional.ofNullable(course).map(Resource::getId)
			.orElse(null);
		final Long seriesId = Optional.ofNullable(series).map(Resource::getId)
			.orElse(null);
	...
	public List<PartialSubmission> getAll(final User user) {
		return Arrays.asList(http.get(
			user.getSubmissionsUrl(), this.client.getApiToken(),
			this.client.getUserAgent(), PartialSubmissionImpl[].class
		));
	}
	...
	public List<PartialSubmission> getAllByMe(final long courseId, final long exerciseId) {
		...
		return Arrays.asList(http.get(
			endpoint, this.client.getApiToken(), this.client.getUserAgent(),
